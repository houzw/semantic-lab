...
import org.semlab.commons.StringUtil;
import org.semlab.ont.query.Ask;
import org.semlab.ont.query.Select;
import org.semlab.ont.rdf.RdfUtil;
import org.semlab.ont.util.Consts;
...
public class SelectImpl implements Select
{
    ...
    public List<Resource> getResourcesByLocalName(Model model, String name)
    {
        if (!StringUtils.isAllLowerCase(name))
            name = name.toLowerCase();// 转为小写：不区分大小写，这样即使输入小写也可以获得本体中包含大写字母的术语。查询中是大小写敏感的
    ...
    public List<String> getSKOSLabelsByLocalName(Model model, String localname)
    {
        if (!StringUtils.isAllLowerCase(localname))
            localname = localname.toLowerCase();// 转为小写：不区分大小写。查询中是大小写敏感的
        ...
        String langTagPattern = "@[a-zA-Z\\-]+";// 语言标签模式，如@cn，@en，@zh-CN等
        while (resultSet.hasNext()) {
            solution = resultSet.next();
